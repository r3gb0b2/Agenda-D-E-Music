...
import { 
  ...
  ChevronLeft,
  List,
  // FIX: Import LayoutDashboard icon
  LayoutDashboard,
                     ...
                     .map(event => (
                       <div key={event.id} className="flex items-center gap-4">
                         <div className="w-12 h-12 bg-slate-800 rounded-lg flex flex-col items-center justify-center font-bold text-primary-400">
                           <span className="text-xs -mb-1">{new Date(event.date).toLocaleDateString('pt-BR', { month: 'short' })}</span>
  ...
  return (
    <>
      ...
      <div className="flex gap-4 overflow-x-auto pb-4">
        {STAGES.map(stage => {
          // FIX: Assign the icon to a capitalized variable name to be used as a JSX component.
